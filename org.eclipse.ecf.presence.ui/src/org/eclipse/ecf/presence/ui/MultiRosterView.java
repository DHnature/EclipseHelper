...
public class MultiRosterView extends ViewPart implements IMultiRosterViewPart {
	...
	private void sendPresence(IPresence.Mode mode) {
		try {
			for (Iterator i = rosterAccounts.iterator(); i.hasNext();) {
				MultiRosterAccount account = (MultiRosterAccount) i.next();
				account.getRosterManager().getPresenceSender().sendPresenceUpdate(null, new Presence(IPresence.Type.AVAILABLE, "test status", mode));
			}
		} catch (ECFException e) {
			Activator.getDefault().getLog().log(e.getStatus());
		}
	...
	private void hookContextMenu() {
		...
		//Commented out the following to suppress additional right-click options.
	...
	private void fillContextMenu(IMenuManager manager) {
		...
		if (element instanceof IRosterEntry) {
			IRosterEntry entry = (IRosterEntry) element;
			final String name = entry.getUser().getName();

			manager.add(imAction);
			imAction.setText("Send Message");
			...
			difficultyDetails.setText("Difficulty Details");
			IAction viewRecording = new Action() {
				public void run() {
					//System.out.println("Requesting video from " + name);
					ServerConnection.getServerConnection().requestVideo(name);
				}
			};
			manager.add(viewRecording);
			viewRecording.setText("View Recording");
			IAction viewScreen = new Action() {
				public void run() {
				}
			};
			manager.add(viewScreen);
			viewScreen.setText("View Screen");
			IAction viewWorkspace = new Action() {
				public void run() {
				}
			};
			manager.add(viewWorkspace);
			viewWorkspace.setText("View Workspace");
